...
class LeaveFilterBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return leavesAsync.when(
      data: (allLeaves) {
        ...
        final filters = <({String label, LeaveFilter filter})>[
          (label: 'All', filter: LeaveFilter.all),
          if (isManagerial) (label: 'Team', filter: LeaveFilter.team),
          (label: 'Pending', filter: LeaveFilter.pending),
          (label: 'Approved', filter: LeaveFilter.approved),
          (label: 'Rejected', filter: LeaveFilter.rejected),
        ];
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: filters.map((f) {
                      final isSelected = selectedFilter == f.filter;
                      return Padding(
              ...
              Row(
                ...
                children: [
                  _buildActionButton(
                    ...
                    onTap: () => _exportToCsv(context, allLeaves),
                  ),
  ...
  Future<void> _exportToCsv(
    ...
    final csvRows = [
      ...
      ...leaves.map(
        (l) => [
          ...
          DateFormat('dd-MM-yyyy').format(l.leaveToDate),
          l.leaveType.name,
          l.leaveApprovalStatus.name,
          l.leaveJustification ?? '',
        ],
      ),
    ];
    ...
    final path =
        '${directory.path}/leaves_export_${DateTime.now().millisecondsSinceEpoch}.csv';
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            hintText: 'Search by name, type, status...',
          ),
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              ...
              if (query.isNotEmpty) {
                // TODO: Update Riverpod state with search query
                ScaffoldMessenger.of(context).showSnackBar(
...
// final leaveFilterProvider = StateNotifierProvider<LeaveFilterNotifier, LeaveFilter>((ref) {
