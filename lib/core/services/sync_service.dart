...
class SyncService {
  ...
  Future<SyncResult> initialSync(String empId) async {
    try {
      ...
      debugPrint("✅ syncEmployeeData completed successfully!");
      // 2. Fetch Mapped Projects & Project Details
      await syncProjects(empId);
      debugPrint("✅ syncProjects completed successfully!");
      // 3. Fetch Attendance Data (current + previous month)
      await syncAttendance(empId);
      debugPrint("✅ syncAttendance completed successfully!");
      // 4. Fetch Regularization Data
      await _fetchAndSaveRegularization(empId);
      debugPrint("✅ _fetchAndSaveRegularization completed successfully!");
      // 5. Fetch Leave Data (when ready)
      await _fetchAndSaveLeaves(empId);

      debugPrint("✅ _fetchAndSaveLeaves completed successfully!");

      await _saveLastSyncTime();
      debugPrint("✅ _saveLastSyncTime completed successfully!");
      return SyncResult(
        success: true,
  ...
  Future<SyncResult> manualSync(String empId) async {
    try {
      ...
      debugPrint("✅ Manual sync for _pushPendingTransactions completed!");
      // Pull latest data from server
      await syncEmployeeData(empId);
      debugPrint("✅ Manual sync for syncEmployeeData completed!");
      await syncProjects(empId);
      debugPrint("✅ Manual sync for syncProjects completed!");
      await syncAttendance(empId);
      debugPrint("✅ Manual sync for syncAttendance completed!");
      await _fetchAndSaveRegularization(empId);
      debugPrint("✅ Manual sync for _fetchAndSaveRegularization completed!");
      await _fetchAndSaveLeaves(empId);

      debugPrint("✅ Manual sync for _fetchAndSaveLeaves completed!");

      await _saveLastSyncTime();
      debugPrint("✅ Manual sync for _saveLastSyncTime completed!");
      return SyncResult(
        success: true,
  ...
  Future<SyncResult> nightlySync(String empId) async {
    try {
      ...
      await syncEmployeeData(empId);
      debugPrint("✅ nightly sync for syncEmployeeData completed!");
      await syncProjects(empId);
      debugPrint("✅ nightly sync for syncProjects completed!");
      await syncAttendance(empId);
      debugPrint("✅ nightly sync for syncAttendance completed!");
      await _fetchAndSaveRegularization(empId);
      debugPrint("✅ nightly sync for _fetchAndSaveRegularization completed!");
      await _fetchAndSaveLeaves(empId);

      debugPrint("✅ nightly sync for _fetchAndSaveLeaves completed!");

      // Cleanup old data
      await _cleanupOldData();
      debugPrint("✅ nightly sync for _cleanupOldData completed!");
      await _saveLastSyncTime();
      debugPrint("✅ nightly sync for _saveLastSyncTime completed!");
        ...
        for (var projectId in projectIds) {
          try {
            ...
            if (projectRes['success'] == true && projectRes['data'] != null) {
              ...
              debugPrint("🔄 Fetching details for $projectData projects");
  ...
  Future<void> syncAttendance(String empId) async {
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        debugPrint("📊 Found-  $records, attendance records");
  ...
  Future<void> _fetchAndSaveRegularization(String empId) async {
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        final regList = response['data']['regularizations'] as List<dynamic>;
        final data = response['data'];

        if (data is List) {
          // OK
        } else if (data is Map && data['regularizations'] is List) {
          // OK
        } else {
          debugPrint("⚠️ Unexpected regularization data format");
          return;
        }
  ...
  Future<void> _fetchAndSaveLeaves(String empId) async {
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        debugPrint("_fetchAndSaveLeaves leaveList : $leaveList");
  ...
  Future<void> _pushPendingTransactions(String empId) async {
    try {
      ...
      debugPrint("_pushPendingTransactions pendingAtt : $pendingAtt");
      ...
      debugPrint("_pushPendingTransactions pendingReg : $pendingReg");
  ...
  Future<String?> _getLastSyncTime() async {
    ...
    debugPrint("_getLastSyncTime result : $result");
