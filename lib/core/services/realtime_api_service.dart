...
class RealtimeApiService {
  ...
  Future<Map<String, dynamic>> checkIn({
    ...
    try {
      ...
      debugPrint("checkin attendanceData: $attendanceData");
  ...
  Future<Map<String, dynamic>> checkOut({
    ...
    try {
      ...
      // Save to local DB
      final attendanceData = response['data'];
      debugPrint("checkOut attendanceData: $attendanceData");
      if (attendanceData != null) {
        final db = await _db.database;
        await db.insert(
          'employee_attendance',
  ...
  Future<Map<String, dynamic>> submitRegularization({
    ...
    try {
      ...
      debugPrint("submitRegularization regData : $regData");
      if (regData != null) {
        final db = await _db.database;
        await db.insert(
  ...
  Future<void> _savePendingRegularization({
    ...
    final db = await _db.database;
    final timestamp = DateTime.now();
    debugPrint("💾 Regularization db : $db");
    await db.insert(
      'employee_regularization',
      {
  ...
  Future<Map<String, dynamic>> applyLeave({
    ...
    try {
      ...
      debugPrint("applyLeave leaveData : $leaveData");
      if (leaveData != null) {
        final db = await _db.database;
        await db.insert(
          'employee_leaves',
  ...
  Future<Map<String, List<Map<String, dynamic>>>> getPendingTransactions(
    ...
    debugPrint("pendingAttendance : $pendingAttendance");
    final pendingReg = await db.query(
      'employee_regularization',
      where: 'emp_id = ? AND (is_synced IS NULL OR is_synced = 0)',
      whereArgs: [empId],
    );
    debugPrint("pendingReg : $pendingReg");
    final pendingLeaves = await db.query(
      'employee_leaves',
      where: 'emp_id = ? AND (is_synced IS NULL OR is_synced = 0)',
      whereArgs: [empId],
    );
debugPrint("pendingLeaves : $pendingLeaves");
    return {
