...
// Fully aligned with dummy_data.dart – ALL tables seeded
// Key Upgrades:
// - All CREATE TABLE statements match dummy_data exactly (no missing columns)
// - Full seeding of ALL tables in _seedDummyData (user, employee_master, project_master, attendance, etc.)
// - Safe inserts with ConflictAlgorithm.replace (no duplicate errors)
// - Better logging & debug prints (only in debug mode)
// - Future-proof onUpgrade with transaction safety
// - Null-safe & optimized (batch inserts where possible)
// - Added missing tables (attendance_analytics, project_manager_mapping, etc.)
// - Ready for production: error handling in seeding
...
class DBHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    final path = join(databasesPath, 'appattendance.db');
  ...
  Future<void> _onCreate(Database db, int version) async {
    // ── CREATE TABLES (exact match with dummy_data.dart) ──────────────────────
    await db.execute('''
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS project_site_mapping (
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS employee_mapped_projects (
        emp_id TEXT NOT NULL,
        project_id TEXT NOT NULL,
...
''');
  ...
  Future<void> _seedDummyData(Database db) async {
    if (kDebugMode) print('Seeding dummy data from dummy_data.dart...');

    try {
      // Insert user
      for (var item in dummyData['user']!) {
        await db.insert(
          'user',
          item,
      ...
      // Insert organization_master
      for (var item in dummyData['organization_master']!) {
        await db.insert(
          'organization_master',
          item,
          conflictAlgorithm: ConflictAlgorithm.replace,
      ...
      for (var item in dummyData['employee_master']!) {
        await db.insert(
          'employee_master',
          item,
          conflictAlgorithm: ConflictAlgorithm.replace,
      ...
      // Insert project_master
      for (var item in dummyData['project_master']!) {
        await db.insert(
          'project_master',
          item,
          conflictAlgorithm: ConflictAlgorithm.replace,
      ...
      // Insert project_site_mapping
      for (var item in dummyData['project_site_mapping']!) {
        await db.insert(
      ...
      for (var item in dummyData['employee_attendance']!) {
        await db.insert(
          'employee_attendance',
          item,
          conflictAlgorithm: ConflictAlgorithm.replace,
      ...
      // Insert employee_regularization
      for (var item in dummyData['employee_regularization']!) {
        await db.insert(
      ...
      // Insert employee_leaves
      for (var item in dummyData['employee_leaves']!) {
        await db.insert(
          'employee_leaves',
          item,
          conflictAlgorithm: ConflictAlgorithm.replace,
      ...
      // Insert employee_mapped_projects
      for (var item in dummyData['employee_mapped_projects']!) {
        await db.insert(
      ...
      // Insert attendance_analytics
      for (var item in dummyData['attendance_analytics']!) {
        await db.insert(
      ...
      if (kDebugMode) {
        print('Dummy data seeding complete! All tables populated.');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> clearAllData() async {
    ...
    await db.execute('DELETE FROM employee_master');
    await db.execute('DELETE FROM project_master');
    await db.execute('DELETE FROM employee_attendance');
    await db.execute('DELETE FROM attendance_analytics');
  ...
  Future<Map<String, dynamic>?> getCurrentUser() async {
    ...
    if (result.isNotEmpty) {
      final userData = result.first['user_data'] as String?;
